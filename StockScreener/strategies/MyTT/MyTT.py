...
def REF(S, N=1):  # 对序列整体下移动N,返回序列(shift后会产生NAN)
    if isinstance(N, np.ndarray):
        ...
        for i in range(len(N)):
            value = pd.Series(S).shift(N[i]).values[i]
...
def WR(CLOSE, HIGH, LOW, N=10, N1=6):  # W&R 威廉指标
    WR_ = (HHV(HIGH, N) - CLOSE) / (HHV(HIGH, N) - LLV(LOW, N)) * 100
    WR1 = (HHV(HIGH, N1) - CLOSE) / (HHV(HIGH, N1) - LLV(LOW, N1)) * 100
    return RD(WR_), RD(WR1)
...
def PSY(CLOSE, N=12, M=6):
    PSY_ = COUNT(CLOSE > REF(CLOSE, 1), N) / N * 100
    PSYMA = MA(PSY_, M)
    return RD(PSY_), RD(PSYMA)
...
def TRIX(CLOSE, M1=12, M2=20):  # 三重指数平滑平均线
    ...
    TRIX_ = (TR - REF(TR, 1)) / REF(TR, 1) * 100
    TRMA = MA(TRIX_, M2)
    return TRIX_, TRMA
...
def EMV(HIGH, LOW, VOL, N=14, M=9):  # 简易波动指标 
    ...
    EMV_ = MA(MID * VOLUME * (HIGH - LOW) / MA(HIGH - LOW, N), N)
    MAEMV = MA(EMV_, M)
    return EMV_, MAEMV


def DPO(CLOSE, M1=20, M2=10, M3=6):  # 区间震荡线
    DPO_ = CLOSE - REF(MA(CLOSE, M1), M2)
    MADPO = MA(DPO_, M3)
    return DPO_, MADPO
...
def MTM(CLOSE, N=12, M=6):  # 动量指标
    MTM_ = CLOSE - REF(CLOSE, N)
    MTMMA = MA(MTM_, M)
    return MTM_, MTMMA


def MASS(HIGH, LOW, N1=9, N2=25, M=6):  # 梅斯线
    MASS_ = SUM(MA(HIGH - LOW, N1) / MA(MA(HIGH - LOW, N1), N1), N2)
    MA_MASS = MA(MASS_, M)
    return MASS_, MA_MASS


def ROC(CLOSE, N=12, M=6):  # 变动率指标
    ROC_ = 100 * (CLOSE - REF(CLOSE, N)) / REF(CLOSE, N)
    MAROC = MA(ROC_, M)
    return ROC_, MAROC
